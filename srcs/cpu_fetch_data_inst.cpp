...
/*puts on fecth_data the correct data we will need for the insctruction depending on the address mode of the instruction*/
void CPU::fetch_data_inst()
{
    switch (curr_inst->mode)
    {
        ...
        case AM_R_D8: //reads a 8bit address from pc
            fetch_data = bus_read(regis.pc, cart_ptr, ram_ptr);
            emu.emu_cycle(1);
            regis.pc++;
            return;

        case AM_R_D16: //reads a 16bit address from pc
        case AM_D16:
        {
            u16 low = bus_read(regis.pc, cart_ptr, ram_ptr);
            emu.emu_cycle(1);
        ...
        case AM_R_MR: //reads from an address and the destination will be the register
        {
            u16 address = cpu_read_regis(curr_inst->reg2);
            if (curr_inst->reg1 == C)
                address |= 0xFF00;
            fetch_data = bus_read(address, cart_ptr, ram_ptr);
            emu.emu_cycle(1);
            return ;
        }
        ...
        {
            fetch_data = bus_read(cpu_read_regis(curr_inst->reg2), cart_ptr, ram_ptr);
            emu.emu_cycle(1);
        ...
        {
            mem_dest = bus_read(regis.pc, cart_ptr, ram_ptr) | 0xFF00;
            dest_is_mem = true;
            emu.emu_cycle(1);
            regis.pc++;
            return ;
        ...
        case AM_A16_R:
        case AM_D16_R: //moving from a register into a 16bit address
        {
            u16 low = bus_read(regis.pc, cart_ptr, ram_ptr);
            emu.emu_cycle(1);
        ...
        case AM_MR:
        {
            mem_dest = cpu_read_regis(curr_inst->reg1);
            dest_is_mem = true;
            fetch_data = bus_read(cpu_read_regis(curr_inst->reg1), cart_ptr, ram_ptr);
            emu.emu_cycle(1);
            return ;
        }
        ...
        default:
            printf("address mode not implemented yet: %X\n", curr_opcode);
